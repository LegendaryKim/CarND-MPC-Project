...
// One variable starts and another ends
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Minimize the offset form the reference state
    for (int t = 0; t < N; t++) {
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  int dim_state_vec = 6;
  int dim_actuator_vec = 2;
  // Set the number of model variables (includes both states and inputs).
  // For example: If the state is a 6 element vector, the actuators is a 2
  // element vector and there are 10 timesteps. The number of variables is:
  //
  // 4 * 10 + 2 * 9
  size_t n_vars = dim_state_vec * N + dim_actuator_vec * (N-1);
  // Set the number of constraints
  size_t n_constraints = dim_state_vec * N;
